...
VCscript_mod <- function(pc_csv_file){
...
pc                <- pc_csv$pc_id
count_pc          <- pc_csv$PCs
count_singleton   <- pc_csv$Singletons
rm(pc_csv)


# Check newtork
vec_in_network  <- which(!is.na(count_pc))
vec_out_network <- which(is.na(count_pc))
removed_contig  <- contig[vec_out_network]
contig          <- contig[vec_in_network]
vc              <- vc[vec_in_network]
pc              <- pc[vec_in_network]
count_pc        <- count_pc[vec_in_network]
count_singleton <- count_singleton[vec_in_network]

# Sorting
vec_order_contig <- order(contig)
contig           <- contig[vec_order_contig]
vc               <- vc[vec_order_contig]
pc               <- pc[vec_order_contig]
count_pc         <- count_pc[vec_order_contig]
count_singleton  <- count_singleton[vec_order_contig]

uniq_contig     <- unique(contig)  # Identify unique contig
uniq_vc         <- unique(vc)
uniq_pc         <- unique(pc)
uniq_pc         <- uniq_pc[order(uniq_pc)]
uniq_pc         <- uniq_pc[uniq_pc != ""]  # remove errors

num_contig     <- length(uniq_contig)
num_vc         <- length(uniq_vc)
num_pc         <- length(uniq_pc)

cat(sprintf("  Total number of contigs in network: %d\n",num_contig))
cat(sprintf("  Total number of contigs out of network: %d\n",length(unique(removed_contig))))
...
num_contig_pc         <- matrix(0,num_contig)
...
for(i in 1:num_contig){
  ...
  if (length(unique(xxx)) > 1){
    cat(sprintf(" For contig: (%d) %s\n",i,uniq_contig[i]))
    stop("STOP!!! More than one PC numbers found")
  }
  num_contig_pc[i] <- xxx[1]

  xxx <- count_singleton[tmp]
  if (length(unique(xxx)) > 1){
    cat(sprintf(" For contig: (%d) %s\n",i,uniq_contig[i]))
    stop("STOP!!! More than one Singleton numbers found")
  }
  num_contig_singletron[i] <- xxx[1]

  pc_contig[match(unique(pc[tmp]),uniq_pc),i] <- 1
  vc_contig[match(unique(vc[tmp]),uniq_vc),i] <- 1
}
num_contig_singletron <- c(num_contig_singletron)
num_contig_pc         <- c(num_contig_pc)
num_singleton <- sum(num_contig_singletron)
